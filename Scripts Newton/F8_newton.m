...
[w_opt, Fval_opt, output] = MN_modificado1(@F8, @F8_with_grad_hess, w0, epsilon, Kmax)

%% Gráfico das curvas de nível (F8)
% Extrair trajetória
traj_w1 = output(:,2);
traj_w2 = output(:,3);
...
% Parâmetro a (conforme definido em F8.m, a=1)
a = 1;
% Expressão vetorizada da função F8
F_vals = (W1-1).^2 + (W2-1).^2 + a*(W1.^2 + W2.^2 - 0.25).^2;

figure
contour(W1, W2, F_vals, 20);
colorbar;
xlabel('w_1'); ylabel('w_2');
title('Método de Newton Modificado - Trajetória (F8)');
hold on
