...
[w_opt, Fval_opt, output] = MN_modificado1(@F3, @F3_with_grad_hess, w0, epsilon, Kmax)

%% Gráfico das curvas de nível (F3)
% Extrair trajetória
traj_w1 = output(:,2);
traj_w2 = output(:,3);
...
% Expressão vetorizada da função F3
F_vals = W1.^2 + 2*W2.^2 - 2*W1.*W2 - 2*W2;

figure
contour(W1, W2, F_vals, 20);
colorbar;
xlabel('w_1'); ylabel('w_2');
title('Método de Newton Modificado - Trajetória (F3)');
hold on
